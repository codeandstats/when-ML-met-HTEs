...
# Create registry -> KEEPS LOGS FOR BATCHTOOLS
if (file.exists(registry_name)) {
  if (OVERWRITE) {
    ...
    reg = makeExperimentRegistry(file.dir = registry_name, packages = packages,
      seed = 123L, work.dir = getwd())
  } else {
    reg = loadRegistry(registry_name, writeable = TRUE, work.dir = getwd())
  }
} else {
  reg = makeExperimentRegistry(file.dir = registry_name, packages = packages,
    seed = 123L)
